...
- Si la contaminación atmosférica (por ejemplo, con el NO₂ o el CO₂) se relaciona con la fertilidad media de los países europeos.
...
1. **Calidad del aire (`DataExtract.csv`)**:  
   A partir de los registros originales de AQ e-Reporting (EEA), limpiamos nombres, homogeneizamos países y contamos observaciones por país–año (`n_obs_aire`).  
   Con ello construimos resúmenes por país (por ejemplo, `total_obs_aire` y `n_anios_aire`).

2. **Fertilidad (Eurostat, anual) (`Fertilidad.csv`)**:  
   Extraemos la **TFR** por país–año, homogeneizamos los códigos `geo` a nombres de país (`pais`) y calculamos resúmenes (por ejemplo, `tfr_media` por país).
...
#Tabla de mapeo de códigos de país (Eurostat -> nombre)

mapa_paises <- tibble::tribble(
  ~geo,        ~pais,
  "AD", "Andorra",
  "AL", "Albania",
  "AM", "Armenia",
...
1. **Limpieza y homogeneización** de ambas tablas (`aire` y `fertilidad`), incluyendo estandarización de nombres de país (`pais`) y conversión de años (`anio`).

2. **Conteo de vigilancia:** construcción de `aire_conteo`, que contiene para cada país–año el número de observaciones de calidad del aire registradas (`n_obs_aire`).

3. **Resumen de fertilidad:** creación de `fert_resumen_nombres`, que incluye los principales estadísticos por país (por ejemplo, `tfr_media`, `anio_min`, `anio_max` y `n_anios`).

4. **Uniones (joins):**
   - `panel_aire_anio_con_fert`: panel país–año donde cada observación de vigilancia se combina con el resumen de fertilidad correspondiente.  
   - `resumen_aire_fert_pais`: resumen combinado por país, utilizado para el **gráfico de dispersión** y los **tests de correlación**.  

```{r pipeline obj1, message = FALSE, warning = FALSE}

#Resumir fertilidad por país
...
# Limpiar y resumir vigilancia de aire (conteo país-año)
...
limpiar_y_contar_aire <- function(aire_raw) {
  aire_raw %>%
    mutate(
      ...
      # Homogeneización de alias frecuentes
      pais = recode(
        pais,
        "Turkey"              = "Türkiye",
...
# Unir resumen de aire y fertilidad por país
unir_aire_y_fertilidad <- function(df_aire, df_fert) {
  df_aire %>%
    full_join(df_fert, by = "pais") %>%
    arrange(pais)
}

#Construcción de datos de fertilidad (Eurostat)
...
fert_paises <- fert_codigos %>%
  left_join(mapa_paises, by = "geo") %>%
  filter(!is.na(pais)) %>%
  select(pais, anio, tfr)

# Resumen general de fertilidad (todos los años disponibles)
fert_resumen_nombres <- resumir_fertilidad(fert_paises)

#Construcción de datos de aire (vigilancia)

aire_conteo <- limpiar_y_contar_aire(aire)

# Resumen de vigilancia de aire por país
aire_resumen_pais <- aire_conteo %>%
  group_by(pais) %>%
  summarise(
    n_anios_aire        = n_distinct(anio),
    anio_min_aire       = min(anio, na.rm = TRUE),
    anio_max_aire       = max(anio, na.rm = TRUE),
    total_obs_aire      = sum(n_obs_aire, na.rm = TRUE),
    media_obs_por_anio  = mean(n_obs_aire, na.rm = TRUE),
    .groups             = "drop"
  )

#Resumen combinado por país (todos los años de fertilidad)
resumen_aire_fert_pais <- unir_aire_y_fertilidad(
  df_aire = aire_resumen_pais,
  df_fert = fert_resumen_nombres
)

#Resumen de fertilidad solo en años con datos de aire (solapamiento)
fert_resumen_solape <- fert_paises %>%
  semi_join(
    aire_conteo %>% distinct(pais, anio),
    by = c("pais", "anio")
  ) %>%
  resumir_fertilidad()

resumen_aire_fert_solape <- unir_aire_y_fertilidad(
  df_aire = aire_resumen_pais,
  df_fert = fert_resumen_solape
)
...
Como primer **“test”**, graficamos `tfr_media` (eje *x*) frente a `total_obs_aire` (eje *y*) por país y ajustamos una **recta OLS** con su **banda de confianza**.  

Este gráfico nos permite explorar de forma descriptiva si existe una **asociación positiva** entre la intensidad de vigilancia del aire y la fertilidad promedio (TFR) en cada país.


```{r grafico1, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}

datos_validos <- resumen_aire_fert_pais %>%
  filter(!is.na(tfr_media), !is.na(total_obs_aire))

datos_validos %>%
  ggplot(aes(x = tfr_media, y = total_obs_aire)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    title = "Observaciones de calidad del aire vs TFR medio (por país)",
    x = "TFR medio",
    y = "Observaciones de calidad del aire (total)"
  )
```

### Resultados:

El diagrama de dispersión entre `total_obs_aire` (y) y `tfr_media` (x), junto con una recta OLS, sugiere que, a grandes rasgos, la asociación es como mucho débil y sensible a *outliers*. Visualmente, la pendiente no parece claramente positiva y la banda de confianza cubre valores cercanos a cero en buena parte del rango de `tfr_media`.

El eje Y (conteos absolutos) probablemente está afectado por el tamaño poblacional, la densidad urbana y el número de estaciones; no es una medida de “intensidad de vigilancia” comparable *per cápita*.

**Lectura honesta.**  
Con este primer gráfico no podemos afirmar que “más vigilancia implica más fertilidad”. Si existe relación, no parece grande y podría estar mediada por terceros factores (riqueza, contaminación, urbanización, gasto sanitario, políticas familiares, etc.).

A continuación realizamos los **tests estadísticos** de Pearson y Spearman para evaluar esta relación de manera más objetiva:

```{r test1, message = FALSE, warning = FALSE}

df_test <- datos_validos %>%
  mutate(
    y_log = log1p(total_obs_aire)  # estabiliza la varianza ante colas largas
  )

cor_pearson  <- cor.test(df_test$tfr_media, df_test$y_log, method = "pearson")
cor_spearman <- cor.test(df_test$tfr_media, df_test$y_log, method = "spearman", exact = FALSE)

resumen_tests <- tibble(
  indicador = c(
    "Correlación Pearson (r)", "p-valor Pearson",
...
knitr::kable(resumen_tests, caption = "Tests estadísticos de asociación entre TFR medio y vigilancia del aire")

```

Los resultados de los tests confirman la lectura visual del gráfico:

**Correlación de Pearson (r = 0.017, p = 0.92)** → no se aprecia relación lineal entre la fertilidad media y el nivel total de observaciones de calidad del aire.

**Correlación de Spearman (ρ = 0.026, p = 0.877)** → tampoco se observa una relación monótona; es decir, los países con más vigilancia no presentan sistemáticamente mayor ni menor fertilidad.
...
En términos sencillos, la intensidad de vigilancia del aire no parece explicar las diferencias de fertilidad entre países europeos. Esto sugiere que la hipótesis naïf (“más vigilancia → más fertilidad”) no se sostiene empíricamente en este corte transversal.
...
oms_europa_limpio <- oms_europa %>%
  mutate(
    pais = str_trim(`WHO Country Name`),
    pais = recode(
      pais,
      "Turkey"              = "Türkiye",
      "Russian Federation"  = "Russia",
...
Tras armonizar y unir ambas tablas mediante un `left_join()`, obtenemos un panel ciudad–país con valores de NO₂ y la fertilidad media asociada a cada país.  
...
co2_limpio <- dioxidocarbono_df %>%
  transmute(
    pais_raw = str_trim(country_name),
    anio     = as.integer(year),
...
ggplot(
  ...
  labs(
    title    = "Ranking de emisiones medias de CO₂ y fertilidad en Europa",
    subtitle = "Línea = emisiones medias de CO₂ (kt) | Color/tamaño = TFR media",
...
p_anim <- ggplot(
  ...
  geom_text(
    ...
    aes(
      x     = -Inf,
      y     = Inf,
      label = sprintf("Beta = %.3f   R² = %.2f   n = %d", beta, r2, n)
    ),
    hjust        = -0.05,
    vjust        = 1.2,
    size         = 3.2,
...
**Pregunta:**
¿Existe una relación entre el peso relativo del PM₂.₅ en la vigilancia del aire y la fertilidad media (TFR) de los países europeos?
...
- Se armonizan los nombres de país (`pais`) igual que en los objetivos anteriores.  
...
Estas conclusiones deben interpretarse con cautela: se trata de un estudio ecológico y descriptivo, basado en datos agregados por país, sin control exhaustivo por posibles factores de confusión y con indicadores ambientales relativamente gruesos (medias nacionales, totales anuales). La evidencia disponible no niega que la calidad del aire afecte a la salud reproductiva, pero sí sugiere que sus efectos sobre la fecundidad observada a nivel país son difíciles de detectar con este tipo de datos. En este sentido, el trabajo funciona como un **punto de partida exploratorio** que señala la necesidad de estudios con mayor resolución espacial, datos individuales y diseños longitudinales o quasi-experimentales para comprender con más precisión los vínculos entre contaminación atmosférica y fertilidad.
