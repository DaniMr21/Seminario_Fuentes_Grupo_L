...
```{r pregunta2_e, message = FALSE, warning = FALSE}
...
<<<<<<< HEAD
  # Se renombra para que coincida con el nombre de columna en stations_meta.
  rename(air_quality_station_eoi_code = air_quality_station_eo_i_code) %>%
=======
  filter(!is.na(air_quality_station_eo_i_code)) %>%
>>>>>>> cc1c1ce692d3c01facc2e118e3ebb3d6e528e9d5
  
  # Unir con los metadatos de las estaciones con el dataframe 'aire'
  left_join(
    stations_meta, 
    by = c("air_quality_station_eo_i_code" = "air_quality_station_eoi_code")
  ) %>%
...
fert_resumen_nombres_renombrado <- fert_resumen_nombres %>%
  rename(country = pais)
resumen_no2_fertilidad <- no2_clasificado_pais %>%
  left_join(fert_resumen_nombres_renombrado, by = "country")
...
```{r grafico5, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}
