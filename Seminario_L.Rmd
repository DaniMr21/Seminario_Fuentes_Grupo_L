...
```{r Grafico2, fig.width=8, fig.height=6, message = FALSE, warning = FALSE, cache=FALSE}
...
```
### Conclusión del Objetivo 1: Vigilancia vs Fertilidad
...
**Pregunta:**
¿Existe una asociación entre los niveles medios de $\text{NO}_2$ en cada país europeo y su fertilidad media?

**Hipótesis:**
Se espera una relación negativa, donde una mayor contaminación media por $\text{NO}_2$ se asocie con TFR más bajas, reflejando el entorno urbano e industrializado que suele caracterizar a los países con baja natalidad.
...
### Análisis Descriptivo I: Gráfico de Burbujas
...
```{r grafico3, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}
library(ggplot2)
library(ggrepel)
...
### Análisis Descriptivo y Prueba Formal de la Asociación $\text{NO}_2$
...
```{r no2_descriptivas, message = FALSE, warning = FALSE}
...
```{r no2_prueba_cor, message = FALSE, warning = FALSE}
...
Exploramos la asociación entre las emisiones de $\text{CO}_2$ y la Tasa de Fecundidad Total (TFR) para evaluar si las estructuras económicas de alto impacto ambiental se correlacionan con los patrones de baja natalidad.

**Pregunta:** ¿Existe una relación entre las Emisiones de $\text{CO}_2$ per cápita en los países europeos y su fertilidad media?

**Hipótesis:** Se espera una relación **negativa**. Los países con mayores emisiones de $\text{CO}_2$ (altamente industrializados/desarrollados) presentarán TFR bajas.
...
### Análisis Descriptivo I: Gráfico de ranking tipo “lollipop” 

La primera estrategia visual para abordar la relación $\text{CO}_2$-Fertilidad es un **gráfico de ranking** que ordena a los países por sus Emisiones Medias de $\text{CO}_2$ a lo largo del periodo de estudio.

Este formato de visualización, permite:

1. **Establecer un ranking** claro para identificar rápidamente a los mayores y menores emisores de $\text{CO}_2$ total.
...
Al ordenar los países por $\text{CO}_2$ y colorearlos por TFR, se puede evaluar visualmente si existe una agrupación sistemática. Si la hipótesis de una relación negativa fuera cierta, esperaríamos que los países con las barras de $\text{CO}_2$ más largas tuvieran puntos más pequeños y de color menos intenso (TFR baja), y viceversa.

```{r grafico4, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}
...
### Análisis Descriptivo II: Gráfico animado CO₂–TFR por año 
...
```{r grafico5, fig.width=8, fig.height=8, message = FALSE, warning = FALSE}
...
### Análisis Descriptivo III: Gráfico de Dispersión
...
```{r grafico6, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}
...
```{r objetivo3_correlacion_formal, message = FALSE, warning = FALSE}
...
print(prueba_cor_co2)
```

#### Conclusión del Objetivo 3: CO₂ vs fertilidad
El análisis formal de la asociación entre las **Emisiones medias de CO**₂ y la Tasa de **Fecundidad Total (TFR)** en los países europeos confirma las impresiones visuales obtenidas en los gráficos. El coeficiente de Pearson ($\rho$) es de **$\mathbf{-0.043}$**, un valor prácticamente nulo que indica que las emisiones de CO₂ no tienen una relación lineal con la TFR. Este resultado contradice la hipótesis de una relación negativa fuerte.
La falta de relación se confirma mediante la prueba de significancia: el **Valor P es de $\mathbf{0.8839}$**, siendo superior al umbral de $\mathbf{0.05}$. Por lo tanto, se concluye formalmente que no existe una asociación estadísticamente significativa entre el CO₂ total y la TFR media en Europa. 

La **hipótesis** de una relación entre el desarrollo industrial y la baja natalidad, medida a través de las emisiones de CO₂, **se rechaza**. Las variaciones en la TFR están impulsadas por otros factores socioeconómicos y culturales que el CO₂ no logra capturar de manera sistemática.
...
## Caracterizar la importancia del $\text{PM}_{2.5}$ dentro de la vigilancia del aire y su relación con la fertilidad
...
**Pregunta:**
¿Existe una relación entre el peso relativo del $\text{PM}_{2.5}$ en la vigilancia del aire y la fertilidad media (TFR) de los países europeos?
...
Después unimos este resumen con `fert_resumen_nombres` para obtener `pm25_share_tfr`, que contiene tanto la proporción de $\text{PM}_{2.5}$ como la fertilidad media (`tfr_media`) por país.
...
```{r PM2.5_por_pais, fig.width=10, fig.height=10, message = FALSE, warning = FALSE}
...
### Análisis Descriptivo I: Gráfico de Barras Ordenadas por Proporción de $\text{PM}_{2.5}$
...
```{r grafico7, fig.width=10, fig.height=10, message = FALSE, warning = FALSE}
...
### Análisis Descriptivo II: Gráfico de Dispersión
...
- Los puntos están esparcidos por todo el gráfico, como si no tuvieran una dirección clara. Hay países con baja prioridad ($\text{PM}_{2.5}$) que tienen TFR alta, y países con alta prioridad ($\text{PM}_{2.5}$) que tienen TFR media.
...
```{r grafico9, fig.width=8, fig.height=6, message = FALSE, warning = FALSE}
...
```
### Conclusión del Objetivo 4: $\text{PM}_{2.5}$ en la vigilancia vs fertilidad
